...
export default function AdminEditBlogPage({ params }: EditBlogPageProps) {
  const { id } = use(params);
  ...
  // Load categories and blog post details
  useEffect(() => {
    const loadData = async () => {
      try {
        // Fetch Categories
        const catQuery = query(collection(db, "categories"), orderBy("createdAt", "asc"));
        ...
        if (blogSnap.exists()) {
          ...
          toast.error("ব্লগ পোস্টটি খুঁজে পাওয়া যায়নি");
          router.push("/admin/blog");
        }
      } catch (err) {
        console.error("Load edit blog page data error: ", err);
        toast.error("প্রয়োজনীয় ডেটা লোড করতে সমস্যা হয়েছে");
      } finally {
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error("কভার ইমেজ আপলোড ব্যর্থ হয়েছে। ফায়ারবেস স্টোরেজ রুলস চেক করুন।");
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // Check Slug Uniqueness only if the slug was modified
      ...
      // Update Firestore Doc — persist sanitized HTML + editor JSON
      ...
      toast.success("ব্লগ পোস্ট সফলভাবে আপডেট হয়েছে");
      router.push("/admin/blog");
    } catch (err) {
      console.error("Update blog post error: ", err);
      toast.error("ব্লগ পোস্ট আপডেট করা যায়নি");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 font-medium">ব্লগ পোস্ট লোড হচ্ছে...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="pb-5 border-b border-gray-200">
        ...
        <h1 className="text-2xl font-bold text-gray-900">ব্লগ পোস্ট সম্পাদনা</h1>
        <p className="text-sm text-gray-500">পূর্বের পোস্টটি পরিবর্তন বা স্ট্যাটাস আপডেট করুন</p>
      </div>
